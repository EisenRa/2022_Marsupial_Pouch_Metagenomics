...
rule all:
    input:
        ...
        expand("2_Reads/{sample}_trimmed_1.fastq.gz", sample=SAMPLE),
        expand("3_Outputs/3_Assemblies/{sample}/{sample}_contigs.fasta", sample=SAMPLE)
################################################################################
...
rule assembly:
    input:
        ...
        non_host_r2 = "3_Outputs/2_Reads/{sample}_non_host_2.fastq.gz"
    output:
        "3_Outputs/3_Assemblies/{sample}/{sample}_contigs.fasta"
    params:
        workdir = "3_Outputs/3_Assemblies/{sample}"
    conda:
        "1_QC_Assembly.yaml"
    threads:
        40
    ...
    shell:
        ...
        metaspades.py \
            ...
            -o {params.workdir}
        mv 3_Outputs/3_Assemblies/{sample}/scaffolds.fasta {output}
            2> {log}
        """
